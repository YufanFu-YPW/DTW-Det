...
from .trans_tools import GaussGTDistributionGenerator
...
@register
class GenerateGaussDistribution(T.Transform):
    """
    将高斯图生成器注册为 Transform 算子
    """
    def __init__(self, img_size=(640, 640), sigma_ratio=1.2, overlay=False, gauss_norm=False) -> None:
        super().__init__()
        # YAML 文件中传入的 img_size 可能是列表 [640, 640]，将其转为元组
        if isinstance(img_size, list):
            img_size = tuple(img_size)
            
        self.generator = GaussGTDistributionGenerator(
            img_size=img_size, 
    ...
    def forward(self, *inputs: Any) -> Any:
        image, target = inputs[0][0], inputs[0][1]
        if 'boxes' in target:
            boxes = target['boxes']
            if len(boxes) > 0:
                gt_distribution_map = self.generator(boxes)
            else:
                gt_distribution_map = torch.zeros((1, self.generator.img_size[0], self.generator.img_size[1]), dtype=torch.float32)
            # 将生成好的高斯图打包装入 target 字典
            target['gt_distribution_map'] = gt_distribution_map
